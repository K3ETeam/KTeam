...
par(mfrow=c(1,2))
hist(oldAges, freq=F, main='Age: Original Data', 
  col='darkgreen', ylim=c(0,0.04))
hist(full$Age, freq=F, main='Age: rpart Output', 
  col='lightgreen', ylim=c(0,0.04))
```

Things look good, let's check for empty values.
...
# Fill empty deck info with a default value 'Z'
# Add 'Z' to Deck factor levels
# levels(full$Deck)<- c(levels(full$Deck),'Z')
# full$Deck[is.na(full$Deck)] <- 'Z'

predicted_deck <- rpart(Deck ~ Pclass + Fare,
                  data=full[!is.na(full$Deck),], 
                  method="class")
full$Deck[is.na(full$Deck)] <- predict(predicted_deck, full[is.na(full$Deck),], type = "class")
```
...
# Now we'll look at the relationship between RPART age & survival
...
full$Child[full$Age < 21] <- 'Child'
full$Child[full$Age >= 21] <- 'Adult'
...
train <- full[1:891,]
test <- full[892:1309,]
trainEmpty <- full[1:891,]
trainEmpty$Survived <- NA
```

## Building the model 

We're going to check RF inputs variables
```{r}
# Show number of missing Pclass values
sum(is.na(full$Pclass))

# Show number of missing Sex values
sum(is.na(full$Sex))

# Show number of missing Age values
sum(is.na(full$Age))

# Show number of missing SibSp values
sum(is.na(full$SibSp))

# Show number of missing Parch values
...
# Show number of missing Title values
sum(is.na(full$Title))

# Show number of missing FSizeD values
sum(is.na(full$FsizeD))

# Show number of missing Child values
sum(is.na(full$Child))

# Show number of missing Mother values
sum(is.na(full$Mother))

# Show number of missing Deck values
sum(is.na(full$Deck))
```
...
# Predict empty train set in order to check aproximate accuracy without kaggle submission
predictionToCheck <- predict(rf_model, trainEmpty)

# Save the solution to a dataframe with two columns: PassengerId and Survived (prediction)
solution <- data.frame(PassengerID = test$PassengerId, Survived = prediction)

# Save empty train solution to a dataframe with two columns: PassengerId and Survived (prediction)
solutionToCheck <- data.frame(PassengerID = train$PassengerId, Survived = predictionToCheck)

# Write the solution to file
solution_check_path <- 'rpart-rf_Solution_check.csv'
write.csv(solution, file = 'rpart-rf_Solution.csv', row.names = F)
write.csv(solutionToCheck, file = solution_check_path, row.names = F)
```
...
```{r}
source("accuracy.R")
accuracy <- evaluateAccuracy(solution_check_path)
accuracy
```
